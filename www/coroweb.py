...
def get(path):
    ...
    '''
    def decorator(func):
    	#装饰器装饰的函数的name、docstring其实已经变化，为了不破坏原函数的name、docstring使用@functools.wrap(func)
...
def add_route(app, fn):
    ...
    #RequestHandler类主要是对handler进行封装，获取request中传入的参数并传入handler中
...
def add_routes(app, module_name):
    ...
    for attr in dir(mod):
        ...
        if callable(fn) and hasattr(fn, '__method__') and hasattr(fn, '__route__'):
            ...
            logging.info('add route %s %s => %s(%s)' % (fn.__method__, fn.__route__, fn.__name__, args))
            add_route(app,fn) 